function SubmitJob(form,preview) {
	if (form.geometryIndex2.value != "") {
		...
		if (form.cartesianCoordinates.checked) getGeometry("GaussianCartesianFormat", "writeUnitCell=true", form.geometry2, false);
	...
	var geometryOptions = "writeUnitCell=true";
	if (getActiveTemplateVariables().includes("isBSSE"))
		geometryOptions += ";bsse=true";
	else if (document.form.nlayer.value > 1)
		geometryOptions += ";qmmm=true";
	if (form.cartesianCoordinates.checked || document.form.nlayer.value > 1)
		getGeometry("GaussianCartesianFormat", geometryOptions, form.geometry, false);
	else
	...
	if (properties.dimFixed > 0 || (properties.dimScanned > 0 && properties.dimOpt > 0)) document.form.redundantInternals.value = 1;
	document.form.dimOpt.value = properties.dimOpt;
	document.form.dimScanned.value = properties.dimScanned;
	document.form.scanVar.value = properties.varScanned;
	document.form.scanStart.value = properties.scanStart;
	document.form.scanStop.value = properties.scanStop;
	document.form.scanSteps.value = properties.scanSteps;
	document.form.scanVar2.value = properties.varScanned2;
	document.form.scanStart2.value = properties.scanStart2;
	document.form.scanStop2.value = properties.scanStop2;
	document.form.scanSteps2.value = properties.scanSteps2;	
...
{
	HandleSemiempirical(form);
	//make sure the "unrestricted" option show up all the time, since this is used implicitly
	templateVariablesStatic.push("unrestricted");
