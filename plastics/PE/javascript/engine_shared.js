...
function getFragments(field) {
	var JSONgeometry = JSON.parse(document.moledit.getGeometry("XYZFormat", ""));
	field.value = JSONgeometry.fragments;
}
...
function ReadJobOptions(form, engine)
{
	...
	for (i = 0; i < job_options.length; i++)
	{
		...
		//save the calculation type (perhaps set from "Other") for later use
		if (key == 'calculation')
		{
	...
	//Handle case of "Other" template
	if (form.templateId.options[form.templateId.selectedIndex].text.substring(0,5) == "Other")
	{
		form.templateId.options[form.templateId.selectedIndex].text = "Other (" + calculation_type + ")";
		form.calculation.value = calculation_type;
	}
...
var templateVariablesStatic = [ 'jobName', 'templateId', 'cartesianCoordinates', 'checkpointFile', 'saveCheckpointFile', 'ppn', 'nodes', 'nodeType', 'gpus', 'resourceList', 'peEnvironment' , 'timeLimit'];
...
function getActiveTemplateVariables()
{
	var templateIndex = document.form.templateId.selectedIndex;
	return templateVariablesUsed[templateIndex];
}
